...
public class HotArticleServiceImpl implements HotArticleService {
    ...
    public void computeHotArticle() {
        ...
        List<ApArticle> arArticleList = apArticleMapper.findArticleListByLast5days(dateParam);

        //2 计算文章分值
        List<HotArticleVo> hotArticleVoList=computeHOtArticle(arArticleList);


        //3 为每一个频道缓存30条发呢之较高的文章
     ...
     * @return
     */
    private List<HotArticleVo> computeHOtArticle(List<ApArticle> arArticleList) {
        List<HotArticleVo> list=new ArrayList<>();
        if(arArticleList!=null && arArticleList.size()>0 ){
            for(ApArticle apArticle: arArticleList){
